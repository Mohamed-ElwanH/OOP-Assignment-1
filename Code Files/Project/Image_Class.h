...
public:
    ...
    ID3D11ShaderResourceView* srv = nullptr;//added this so the ui doesn't have to regenerate the image every frame
    ...
    Image(int mWidth, int mHeight) {
        this->width = mWidth;
        this->height = mHeight;
        this->channels = 4; // Always RGBA for DirectX safety

        this->imageData = (unsigned char*)malloc(width * height * channels);
        if (!this->imageData) {
            throw std::bad_alloc();
        }

        // Initialize with black + full alpha
    ...
    Image& operator=(const Image& image) {
        ...
        this->channels = (image.channels < 4) ? 4 : image.channels; // Always at least 4
        ...
        // Copy pixel data safely, pad alpha if needed
        if (image.channels == 4) {
            std::memcpy(this->imageData, image.imageData, (size_t)width * height * 4);
        }
        else if (image.channels == 3) {
            for (int i = 0; i < width * height; ++i) {
                this->imageData[i * 4 + 0] = image.imageData[i * 3 + 0];
                this->imageData[i * 4 + 1] = image.imageData[i * 3 + 1];
                this->imageData[i * 4 + 2] = image.imageData[i * 3 + 2];
                this->imageData[i * 4 + 3] = 255; // Fill alpha
            }
        }
    ...
    bool loadNewImage(const std::string& filename) {
        ...
        //imageData = stbi_load(filename.c_str(), &width, &height, &channels, STBI_rgb_alpha); //I did it to force RGBA
        ...
        //********************************************CONVERTING RGB TO RGBA SO THAT DIRECTX CAN USE IT******************************************
    ...
    bool saveImage(const std::string& outputFilename) {
        ...
        if (extensionType == PNG_TYPE) {
            stbi_write_png(outputFilename.c_str(), width, height, channels, imageData, stride_in_bytes);
        }
        else if (extensionType == BMP_TYPE) {
            stbi_write_bmp(outputFilename.c_str(), width, height, channels, imageData);
        }
        else if (extensionType == TGA_TYPE) {
            stbi_write_tga(outputFilename.c_str(), width, height, channels, imageData);
        }
        else if (extensionType == JPG_TYPE) {
            stbi_write_jpg(outputFilename.c_str(), width, height, channels, imageData, 90);
        }
